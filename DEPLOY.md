...
Если бот уже создан (шаг 1) и домен уже указывает на сервер (шаг 2),
...
curl -fsSL https://raw.githubusercontent.com/denis313/KBGU/claude/nifty-johnson-63kr58/deploy/install.sh \
  | DOMAIN=ваш-домен.ru bash
```

Скрипт `deploy/install.sh` спросит токен бота (ввод скрыт). Затем он:

1. проверит, что домен указывает на этот сервер;
2. установит Docker;
3. скачает проект в `/opt/calorie-tracker`;
4. создаст `.env` со случайными секретами;
5. запустит базу, приложение и HTTPS;
6. дождётся, пока сайт откроется, и зарегистрирует бота в Telegram.

Повторный запуск той же команды обновляет приложение, а секреты и данные
сохраняет. После установки в папке `/opt/calorie-tracker` можно писать просто
`docker compose ...`, без `-f docker-compose.prod.yml`.

Шаги ниже описывают то же самое вручную. Они пригодятся, если что-то пошло не так.
